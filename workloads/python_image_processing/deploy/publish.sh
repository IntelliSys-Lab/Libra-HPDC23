...
virtualenvPath=`cat $config | jq '.virtualenvPath' | tr -d '"'`
...
then
	echo
	echo "----- Deploying onto Apache OpenWhisk -----"
	echo
	...
	then
		cp -R $virtualenvPath ./build
	fi

	cd ./build
	zip -X -r ./index.zip *
	
	$wsk action update $function --kind python:3 --main $ibmHandler --memory $memory index.zip
	cd ..

	echo
	echo Testing function on Apache OpenWhisk...

	param_num=`echo $ibmjson | wc -w`
	ibmparams=""
	for i in $(seq 1 2 $param_num)
	do
	    key=$(echo $ibmjson | awk '{print $'$i'}')
	    value=$(echo $ibmjson | awk '{print $'$(($i+1))'}' | tr -d ',')
	    ibmparams="$ibmparams -p $key $value"
	done
	
	$wsk action invoke $function $ibmparams 
	echo
fi
