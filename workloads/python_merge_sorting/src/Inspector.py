        ...
        if (key != None):
            ...
            if (key != None):
                ...
                if (key != None):
                    ...
                    if "ibm" in self.__attributes['functionRegion']:
                        self.__attributes['platform'] = "IBM Cloud Functions"
                        self.__attributes['functionName'] = key
                        self.__attributes["vmID"] = self.runCommand("cat /sys/hypervisor/uuid").strip()
                    else: # Apache OpenWhisk
                        self.__attributes['platform'] = "Apache OpenWhisk"
                        self.__attributes['functionName'] = key
                        self.__attributes["containerID"] = self.runCommand("cat /etc/hosts | tail -1 | awk {'print $2'}").strip()
                        self.__attributes["invokerID"] = self.runCommand("curl http://169.254.169.254/latest/meta-data/instance-id").strip()
                else:
