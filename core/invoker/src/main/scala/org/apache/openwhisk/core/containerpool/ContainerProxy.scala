...
import org.apache.openwhisk.core.containerpool.docker.{ProcessRunner, ProcessUnsuccessfulException}
...
case class Run(action: ExecutableWhiskAction, msg: ActivationMessage, retryLogDeadline: Option[Deadline] = None) {
  var predictPeak: Int = 0
  var predictDuration: Long = 0.toLong
  var actualCpu: Int = 0
  var actualMemory: Int = 0
  var startTime: Long = 0.toLong
}
case object Remove
...
case class RunCompleted(job: Run)
case class UpdateContainer(r: Run, container: Container)
case class Safeguard(r: Run, container: Container)
...
class ContainerProxy(factory: (TransactionId,
  ...
  class SafeguardThread(
    ...
    val mem_actual = job.actualMemory
    val cpu_actual = job.actualCpu
    val mem_total_file = s"/sys/fs/cgroup/memory/docker/${container.containerId.asString}/memory.usage_in_bytes"
    val mem_cache_file = s"/sys/fs/cgroup/memory/docker/${container.containerId.asString}/memory.stat"
    val cpu_cgroup_file = s"/sys/fs/cgroup/cpuacct/docker/${container.containerId.asString}/cpuacct.usage_percpu"
    val cpu_system_file = s"/proc/stat"
    var isSafeguard: Boolean = false

    val read_usage = new {} with ProcessRunner {
      def execute(): Unit = {
        val prev_cpu_cgroup_arr = Await.result(executeProcess(Seq("cat", cpu_cgroup_file), Duration.Inf), Duration.Inf).split(" ")
        val prev_cpu_system_arr = Await.result(executeProcess(Seq("cat", cpu_system_file), Duration.Inf), Duration.Inf).split("\n")(0).split("  ")(1).split(" ")
        var prev_cpu_cgroup: Long = 0
        var prev_cpu_system: Long = 0
        prev_cpu_cgroup_arr.map{case v => prev_cpu_cgroup = prev_cpu_cgroup + v.toLong}
        prev_cpu_system_arr.map{case v => prev_cpu_system = prev_cpu_system + v.toLong}

        Thread.sleep(interval)

        val after_cpu_cgroup_arr = Await.result(executeProcess(Seq("cat", cpu_cgroup_file), Duration.Inf), Duration.Inf).split(" ")
        val after_cpu_system_arr = Await.result(executeProcess(Seq("cat", cpu_system_file), Duration.Inf), Duration.Inf).split("\n")(0).split("  ")(1).split(" ")
        var after_cpu_cgroup: Long = 0
        var after_cpu_system: Long = 0
        after_cpu_cgroup_arr.map{case v => after_cpu_cgroup = after_cpu_cgroup + v.toLong}
        after_cpu_system_arr.map{case v => after_cpu_system = after_cpu_system + v.toLong}

        val delta_cpu_cgroup = after_cpu_cgroup - prev_cpu_cgroup
        val delta_cpu_system = (after_cpu_system - prev_cpu_system) * 1e9 / 100
        val online_cpus = prev_cpu_cgroup_arr.length.max(after_cpu_cgroup_arr.length)
        val cpu_busy = delta_cpu_cgroup / delta_cpu_system * online_cpus

        val mem_total: Long = Await.result(executeProcess(Seq("cat", mem_total_file), Duration.Inf), Duration.Inf).toLong
        val mem_cache_arr = Await.result(executeProcess(Seq("cat", mem_cache_file), Duration.Inf), Duration.Inf).split("\n")
        val mem_cache: Long = mem_cache_arr(mem_cache_arr.length - 3).split(" ")(1).toLong
        val memory_busy = (mem_total - mem_cache) / 1e6 // megabytes

        // If at least one of the two resources is harvested and it's exceeding threshold, trigger safeguard for the invocation
        if ((mem_actual < mem_user && memory_busy >= mem_actual*threshold) || (cpu_actual < cpu_user && cpu_busy >= cpu_actual*threshold)) {
          logging.warn(this, s"Trigger safeguard for job ${job}! memory usage ${memory_busy}, memory limit ${mem_actual*threshold}; cpu usage ${cpu_busy}, cpu limit ${cpu_actual*threshold}")
          implicit val tid = job.msg.transid
          container.update(cpu_user, mem_user.MB)
          logging.info(this, s"safeguard update job ${job}, container ${container} with cpu ${cpu_user} and memory ${mem_user.MB}")
          isSafeguard = true
        }
      }
    }
      ...
      breakable {
        while (true) {
          try {
            ...
            read_usage.execute()
            if (isSafeguard) {
              context.parent ! Safeguard(job, container)
              break
            }
          } catch {
            case e: InterruptedException => 
            case e: ProcessUnsuccessfulException => break
          }
        }
      }
    }
  ...
  var safeguard: Option[SafeguardThread] = None
  val safeguardInterval: Int = 100 // 100 ms
  val safeguardThreshold: Double = 0.8
  ...
  when(Uninitialized) {
    ...
    case Event(job: Run, _) =>
      val memory = job.actualMemory
      val cpu = job.actualCpu
      ...
      container
        .andThen {
          case Success(container) =>
            ...
            if (MemoryLimit.encode(memory, cpu) <= 0) {
              logging.error(this, s"PreWarmedData create error, got memory ${memory}, cpu ${cpu}, translate to limit ${MemoryLimit.encode(memory, cpu)}")
            }
            self ! PreWarmCompleted(
              PreWarmedData(container, job.action.exec.kind, MemoryLimit.encode(memory, cpu).MB, 1))
            // Send the container back to container pool
            context.parent ! UpdateContainer(job, container)
  ...
  when(Running) {
    ...
    // Run was successful
    case Event(RunCompleted(job: Run), data: WarmedData) =>
      // Notify the container pool about the completion of this job
      context.parent ! RunCompleted(job)
  ...
  def initializeAndRun(container: Container, job: Run, reschedule: Boolean = false)(
    ...
    // Start the safeguard daemon, only spy on harvested invocations
    val (userMemory, userCpu) = (MemoryLimit.decodeMemory(job.action.limits.memory.megabytes), MemoryLimit.decodeCpu(job.action.limits.memory.megabytes))
    if (userMemory > job.actualMemory || userCpu > job.actualCpu) {
      if (safeguard.isDefined) {safeguard.get.interrupt()}
      safeguard = Some(new SafeguardThread(
        interval=safeguardInterval,
        threshold=safeguardThreshold,
        job=job,
        container=container
      ))
      safeguard.get.start()
    ...
    // Stop the safeguard daemon if defined
    if (safeguard.isDefined) {
      safeguard.get.interrupt()
      safeguard = None
    }
