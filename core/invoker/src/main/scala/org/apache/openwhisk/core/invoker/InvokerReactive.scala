...
class InvokerReactive(
  ...
  // private val maximumContainers = (poolConfig.userMemory / (MemoryLimit.MIN_MEMORY * MemoryLimit.MEM_UNIT)).toInt
  private val maximumContainers = (MemoryLimit.USER_CPU / MemoryLimit.MIN_CPU).toInt
  ...
  private val pool =
    actorSystem.actorOf(ContainerPool.props(topic, childFactory, poolConfig, activationFeed, prewarmingConfigs))
