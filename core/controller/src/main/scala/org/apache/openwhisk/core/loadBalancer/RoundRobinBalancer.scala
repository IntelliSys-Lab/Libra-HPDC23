...
class RoundRobinBalancer(
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val chosen = if (invokersToUse.nonEmpty) {
      val invoker: Option[(InvokerInstanceId, Boolean)] = RoundRobinBalancer.schedule(
...
object RoundRobinBalancer extends LoadBalancerProvider {
  ...
  private val invokerIndexSum: AtomicInteger = new AtomicInteger(0)
  ...
  def schedule(
    ...
    if (numInvokers > 0) {
      val invokerToChoose: Int = invokerIndexSum.getAndIncrement() % numInvokers
      val invoker = invokers(invokerToChoose)
      ...
      if (invoker.status.isUsable && dispatched(invoker.id.toInt).tryAcquireConcurrent(fqn, maxConcurrent, memorySlots, cpuSlots)) {
        ...
        if (stepsDone == numInvokers + 1) {
          ...
          schedule(maxConcurrent, fqn, invokers, dispatched, slots, stepsDone + 1)
        }
      }
    } else {
      None
