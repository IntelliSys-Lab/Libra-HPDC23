...
class MWSBalancer(
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    // Fetch MWS load from redis
    val mwsLoadList: IndexedSeq[Double] = redis.getMWSLoadForInvoker(invokersToUse.size)
    if (mwsLoadList.length == 0) {
      logging.error(this, s"failed to fetch MWS load list from redis")
    }

    val chosen = if (invokersToUse.nonEmpty) {
      val invoker: Option[(InvokerInstanceId, Boolean)] = MWSBalancer.schedule(
...
object MWSBalancer extends LoadBalancerProvider {
  ...
  def schedule(
    ...
    if (numInvokers > 0) {
      mwsLoadList.zipWithIndex.sortBy(_._1).map{
        case (_, index) => {
