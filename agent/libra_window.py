    ...
    for exp_id in params.EXP_EVAL:
        ...
        for episode_per_exp in range(params.MAX_EPISODE_EVAL):
            ...
            while episode_done is False:
                ...
                # Update windows
                window[current_function_id] = env.request_record.get_last_n_done_request_per_function(current_function_id, window_size)

                # Determine action
                if len(window[current_function_id]) == 0:
                    action = {
                    ...
                    total_memory_peak = 0
                    total_duration = 0
                    for request in window[function_id]:
                        total_cpu_peak = total_cpu_peak + request.get_cpu_peak()
                        total_memory_peak = total_memory_peak + request.get_mem_peak()
                        total_duration = total_duration + request.get_duration()

                    avg_cpu_peak = total_cpu_peak / window_size
                    avg_memory_peak = total_memory_peak / window_size
                    avg_duration = total_duration / window_size
                    
                    # Max values from recent invocations
                    action = {
                        "cpu": USER_CONFIG[current_function_id]["cpu"] if avg_cpu_peak == 0 else int(avg_cpu_peak)+1, 
                        "memory": USER_CONFIG[current_function_id]["memory"] if avg_memory_peak == 0 else int(avg_memory_peak/params.MEMORY_UNIT)+1,
...
if __name__ == "__main__":
    ...
    window_rm(logger_wrapper=logger_wrapper)
